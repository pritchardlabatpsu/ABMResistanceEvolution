...
mut = 1e-7;         % mutation rate [/division]
.
...
parfor pat = 1:npats
    ...
    epsilon = .03;  % error tolerance in tau-leaping (used to calculate tau, see calculations in Cao Journal of Chemical Physics 2006 for more details)
        ...
        if isnan(nSSAruns)
            ...
            if isnan(tau)
                ...
                Jac = [diag(b(1:3)) zeros(3,1) % division
                       2*pop_curr(1)/pop_curr(4) 0 0 -pop_curr(1)^2/pop_curr(4)^2
                       diag(d)
                       diag(alpha)
                       mut*b(1) 0 0 0
                       rec rec rec 0];
    ...
    disp(pat)
    writematrix([t' pop],strcat('WellMixed_adjfrac0.1_pat',num2str(pat),'_011222.csv'))
